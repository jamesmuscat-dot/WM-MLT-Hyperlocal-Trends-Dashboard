...
return resolved
return None
def top_metric_card(label, value, subtitle, icon=None, icon_bg="
#EEF0FF", icon_color="
#6F5CFF", value_color="
#2f3240", card_bg="
#ffffff"):
...
if icon:
icon_html = f"""
<div style=" width: 56px; height: 56px; min-width: 56px; border-radius: 14px; background: {icon_bg}; color: {icon_color}; display: flex; align-items: center; justify-content: center; flex-shrink: 0; ">{icon}</div>
"""
return f"""
<div style=" background: {card_bg}; border-radius: 16px; box-shadow: 0 8px 20px rgba(17, 24, 39, 0.06); padding: 16px 18px; display: flex; align-items: center; gap: 14px; height: 116px; overflow: hidden; ">
{icon_html}
<div style="min-width: 0;">
<div style="font-size: 13px; color: 
#6b7280; margin-bottom: 6px; font-weight: 600;">
{esc(label)}
</div>
...
#ffffff; border-radius: 14px; padding: 10px 12px; margin-bottom: 8px; box-shadow: 0 8px 18px rgba(17, 24, 39, 0.06); ">
...
social_norm = minmax_score(social_raw)
hidden_reviews = reviews.fillna(reviews.max() if reviews.notna().any() else 0)
hidden_review_norm = minmax_score(np.log1p(hidden_reviews))
...
icons_html.append(f'<span style="font-size:13px; color:
#4b5563;">{html.escape(name)}</span>')
if not icons_html:
...
if profile_pic_uri:
pic_html = f"""
<img src="{profile_pic_uri}" style=" width:56px; height:56px; border-radius:999px; object-fit:cover; box-shadow:0 6px 14px rgba(17,24,39,0.12); ">
"""
else:
pic_html = """
<div style=" width:56px; height:56px; border-radius:999px; background:
#e9ecf7; "></div>
"""
...
f'style="color:
#6F5CFF; font-weight:600; text-decoration:none;">View profile ↗</a>'
)
else:
...
#2f3240; line-height:1.2;">{html.escape(name_part)}</div>
<div style="font-size:13px; color:
#6b7280; margin-top:3px;">{html.escape(handle_part)}</div>
</td>
...
#e7eaf3; border-radius:16px; overflow:hidden; box-shadow:0 8px 18px rgba(17,24,39,0.05); ">
<div style="max-height:430px; overflow:auto;">
<table style=" width:100%; border-collapse:collapse; table-layout:fixed; ">
<thead>
<tr style=" background:
#fafbfe; color:
#8a8f9c; font-size:14px; font-weight:700; border-bottom:1px solid 
#e7eaf3; ">
...
"min-width:44px; border-radius:999px; background:
#6F5CFF; color:white; flex-shrink:0;"
)
return f"""
<div>
<div style="display:flex; align-items:center; gap:8px;">
<div style="font-size:28px; font-weight:800; color:
#2f3240;">Match Your Catalogue to These Trends</div>
<div style="color:
#9096a3;">{ICON_LINK}</div>
</div>
<div style="font-size:14px; color:
#6b7280; margin-top:4px; margin-bottom:16px;">
Use AI to match your candidate products to the latest food & drink trends and get a ranked shortlist.
</div>
<div style=" background:
#ffffff; border:1px solid 
#ECEEF3; border-radius:16px; box-shadow:0 8px 20px rgba(17,24,39,0.05); padding:18px 22px; margin-bottom:16px; display:flex; align-items:center; justify-content:space-between; gap:10px; ">
...
<div style="min-width:0;">
<div style="font-weight:700; color:
#2f3240; font-size:14px;">1. Copy the prompt</div>
<div style="font-size:12px; color:
#8a8f9c; margin-top:2px;">We've prepared a detailed prompt with the latest trend insights.</div>
</div>
</div>
<div style="color:
#c3c7d1; font-size:20px; padding:0 6px;">→</div>
<div style="display:flex; align-items:center; justify-content:center; gap:14px; flex:1; min-width:0;">
<div style="{step_icon_wrap}">{ICON_CLIPBOARD_COPY}</div>
<div style="min-width:0;">
<div style="font-weight:700; color:
#2f3240; font-size:14px;">2. Paste into AI</div>
<div style="font-size:12px; color:
#8a8f9c; margin-top:2px;">Open Claude or ChatGPT and paste the prompt.</div>
</div>
</div>
<div style="color:
#c3c7d1; font-size:20px; padding:0 6px;">→</div>
...
<div style="min-width:0;">
<div style="font-weight:700; color:
#2f3240; font-size:14px;">3. Upload your catalogue</div>
<div style="font-size:12px; color:
#8a8f9c; margin-top:2px;">Upload your candidate catalogue (CSV or Excel) and get results.</div>
</div>
</div>
</div>
<div style=" background:
#FFF7E0; border:1px solid 
#F5E6B8; border-bottom:none; border-radius:16px 16px 0 0; padding:14px 20px; display:flex; align-items:center; gap:8px; font-weight:700; color:
#9A7B1F; font-size:17px; ">
<span style="color:
#F5A623;">{ICON_SPARKLES}</span>
...
if icon_uri:
return f"""
<div style=" width:40px; height:40px; min-width:40px; border-radius:999px; background:
#f0f1f5; display:flex; align-items:center; justify-content:center; flex-shrink:0; overflow:hidden; ">
<img src="{icon_uri}" alt="{html.escape(name)}" style=" width:100%; height:100%; object-fit:cover; object-position:center; transform:scale({zoom}); transform-origin:{origin}; ">
</div>
"""
return f"""
<div style=" width:40px; height:40px; border-radius:999px; background:{fallback_bg}; color:white; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:16px; flex-shrink:0; ">{fallback_letter}</div>
"""
claude_icon_html = _assistant_icon_html("Claude", "
#D97757", "C", zoom=1.0)
gpt_icon_html = _assistant_icon_html("ChatGPT", "
#10A37F", "G", zoom=2.3, origin="50% 43%")
return f"""
<div>
<div style="background:
#F5F3FF; border:1px solid 
#E2DBFF; border-radius:16px; padding:16px 20px; text-align:center; margin-bottom:14px;">
<div style="font-weight:700; color:
#6F5CFF; font-size:15px;">Ready to get your ranked shortlist?</div>
<div style="font-size:13px; color:
#6b7280; margin-top:2px;">Choose your preferred AI assistant to continue.</div>
</div>
<div style="display:flex; gap:14px;">
<a href="https://claude.ai/new" target="_blank" rel="noopener noreferrer" style=" flex:1; display:flex; align-items:center; gap:12px; text-decoration:none; background:
#ffffff; border:1px solid 
#ECEEF3; border-radius:14px; padding:14px 16px; box-shadow:0 6px 16px rgba(17,24,39,0.05); ">
{claude_icon_html}
<div style="min-width:0;">
<div style="font-weight:700; color:
#2f3240; font-size:14px;">Open in Claude</div>
<div style="font-size:12px; color:
#8a8f9c; margin-top:2px;">Paste the prompt and upload your catalogue</div>
</div>
</a>
<a href="https://chat.openai.com/" target="_blank" rel="noopener noreferrer" style=" flex:1; display:flex; align-items:center; gap:12px; text-decoration:none; background:
#ffffff; border:1px solid 
#ECEEF3; border-radius:14px; padding:14px 16px; box-shadow:0 6px 16px rgba(17,24,39,0.05); ">
{gpt_icon_html}
<div style="min-width:0;">
<div style="font-weight:700; color:
#2f3240; font-size:14px;">Open in ChatGPT</div>
<div style="font-size:12px; color:
#8a8f9c; margin-top:2px;">Paste the prompt and upload your catalogue</div>
</div>
</a>
...
#EDEBFF; color:
#6F5CFF; font-size:12px; font-weight:600; padding:5px 12px; border-radius:999px; margin:0 6px 6px 0; ">{html.escape(c)}</span>"""
for c in chips
)
return f"""
<div style=" background:
#ffffff; border:1px solid 
#ECEEF3; border-radius:16px; box-shadow:0 8px 20px rgba(17,24,39,0.05); padding:18px 22px; margin-bottom:8px; ">
<div style="display:flex; align-items:center; gap:8px; margin-bottom:10px;">
<span style="color:
#6F5CFF;">{ICON_MAP_PIN}</span>
...
<div style="font-size:14px; color:
#4b5563; line-height:1.6; margin-bottom:12px;">
{html.escape(display_value(row.get("Summary")))}
</div>
<div style="display:flex; flex-wrap:wrap; gap:16px; margin-bottom:12px;">
<div style="flex:1; min-width:180px;">
<div style="font-size:12px; color:
#9096a3; font-weight:600; margin-bottom:2px;">DOMINANT SEGMENTS</div>
<div style="font-size:13px; color:
#2f3240;">{html.escape(display_value(row.get("Dominant Segments")))}</div>
</div>
<div style="flex:1; min-width:180px;">
<div style="font-size:12px; color:
#9096a3; font-weight:600; margin-bottom:2px;">NOTABLE COMMUNITIES</div>
<div style="font-size:13px; color:
#2f3240;">{html.escape(display_value(row.get("Notable Communities")))}</div>
</div>
<div style="flex:1; min-width:180px;">
<div style="font-size:12px; color:
#9096a3; font-weight:600; margin-bottom:2px;">SPENDING PROFILE</div>
<div style="font-size:13px; color:
#2f3240;">{html.escape(display_value(row.get("Spending Profile")))}</div>
</div>
</div>
<div style="font-size:12px; color:
#9096a3; font-weight:600; margin-bottom:6px;">SUGGESTED RANGE FOCUS</div>
<div>{chips_html}</div>
<div style="font-size:11px; color:
#b3b8c2; margin-top:12px;">
...
#2f3240;">{html.escape(tier)}</span>
</td>
<td style="padding:8px 6px; font-size:13px; color:
#4b5563;">{html.escape(bracket["weekly"])}</td>
<td style="padding:8px 6px; font-size:13px; color:
#4b5563;">{html.escape(bracket["basket"])}</td>
</tr>
"""
...
<tr style="border-bottom:1px solid 
#ECEEF3;">
<th style="text-align:left; padding:6px; font-size:11px; color:
#9096a3; font-weight:600;">TIER</th>
<th style="text-align:left; padding:6px; font-size:11px; color:
#9096a3; font-weight:600;">WEEKLY FOOD SPEND</th>
<th style="text-align:left; padding:6px; font-size:11px; color:
#9096a3; font-weight:600;">BASKET VALUE</th>
</tr>
</thead>
...
f"""<span style=" display:inline-block; background:
#F5F3FF; color:
#6F5CFF; font-size:11px; font-weight:700; padding:4px 10px; border-radius:999px; margin:0 6px 6px 0; border:1px solid 
#E2DBFF; ">{html.escape(t)}</span>"""
for t in tags
)
confidence = display_value(row.get("Confidence"))
confidence_style = CONFIDENCE_BADGE_STYLE.get(confidence, "background:
#EEF0FF; color:
#6F5CFF;")
detail_fields = [
...
<div style="font-size:11px; color:
#9096a3; font-weight:600; margin-bottom:2px;">{label}</div>
<div style="font-size:13px; color:
#2f3240;">{html.escape(display_value(value))}</div>
</div>
"""
...
<div style=" background:
#ffffff; border:1px solid 
#ECEEF3; border-radius:18px; box-shadow:0 10px 26px rgba(17,24,39,0.06); padding:22px 26px; ">
...
<span style=" display:inline-block; padding:5px 12px; border-radius:999px; font-size:12px; font-weight:700; {confidence_style} ">Confidence: {html.escape(confidence)}</span>
</div>
<div style="font-size:14px; color:
#4b5563; line-height:1.6; margin:10px 0 14px 0;">
{html.escape(display_value(row.get("Summary")))}
</div>
...
<div style="font-size:12px; color:
#9096a3; font-weight:600; margin-bottom:6px;">QUICK TAGS</div>
<div style="margin-bottom:12px;">{tags_html if tags_html else '<span style="font-size:12px; color:
#b3b8c2;">No strong tags detected</span>'}</div>
<div style="font-size:12px; color:
#9096a3; font-weight:600; margin-bottom:6px;">SUGGESTED RANGE FOCUS</div>
<div>{chips_html}</div>
<div style="font-size:11px; color:
#b3b8c2; margin-top:14px;">
AI-researched overview — directional only, not verified statistics. Spot-check before using for sourcing decisions.
</div>
</div>
"""
...
image_html = """
<div style=" width:96px; height:96px; border-radius:999px; background:
#e9ecf7; flex-shrink:0; "></div>
"""
image_uri = image_path_to_data_uri(image_path) if image_path else None
if image_uri:
image_html = f"""
<img src="{image_uri}" style=" width:96px; height:96px; border-radius:999px; object-fit:cover; flex-shrink:0; box-shadow:0 8px 18px rgba(17,24,39,0.12); ">
"""
strength_style = strength_chip_style(strength)
return f"""
<div style=" display:flex; gap:14px; align-items:flex-start; background:
#ffffff; border:1px solid 
#e7eaf3; border-radius:16px; padding:14px 16px; box-shadow: 0 8px 18px rgba(17,24,39,0.05); margin-bottom:14px; ">
<div style=" width:30px; height:30px; border-radius:8px; background:
#4caf50; color:white; display:flex; align-items:center; justify-content:center; font-size:15px; font-weight:800; flex-shrink:0; margin-top:4px; ">{int(rank)}</div>
{image_html}
...
<div style=" display:inline-block; padding:6px 14px; border-radius:999px; font-size:14px; font-weight:700; {strength_style} ">{html.escape(display_value(strength))}</div>
</div>
<div style="margin-top:8px; font-size:14px; line-height:1.55; color:
#4b5563;">
...
with kpi1:
st.markdown(clean_html(top_metric_card("Total Producers", len(filtered_df), "Across all neighbourhoods", icon=ICON_USERS, icon_bg="
#EDEBFF", icon_color="
#6F5CFF", card_bg="
#F5F3FF")), unsafe_allow_html=True)
with kpi2:
st.markdown(clean_html(top_metric_card("Neighbourhoods", filtered_df["Neighbourhood"].nunique(), "Across Malta", icon=ICON_MAP_PIN, icon_bg="
#E5F0FF", icon_color="
#2F6BFF", card_bg="
#EFF6FF")), unsafe_allow_html=True)
with kpi3:
st.markdown(clean_html(top_metric_card("Categories", filtered_df["Category"].nunique(), "Artisanal categories", icon=ICON_GRID, icon_bg="
#E6F7EC", icon_color="
#48B26B", card_bg="
#F0FBF3")), unsafe_allow_html=True)
with kpi4:
st.markdown(clean_html(top_metric_card("Mapped Rows", len(filtered_map_df), "Have coordinates", icon=ICON_MAP, icon_bg="
#FFF3E0", icon_color="
#F4A94E", card_bg="
#FFFBF0")), unsafe_allow_html=True)
...
"Strength": "Strong",
"Description": "Rising matcha demand in Malta across cafés and retail. Translating into latte kits, powders, syrups, and dessert applications.",
"Image": "matcha.jpg",
},
{
"Rank": 2,
...
"Strength": "Strong",
"Description": "Maltese appetite for pistachio and kunefe treats continues to grow. Opportunity in spreads, fillings, bakery SKUs, and premium desserts.",
"Image": "pistachio.jpg",
},
{
"Rank": 3,
...
with strength_col1:
st.markdown(clean_html(top_metric_card("Top Trend Strength", top_strength, "Matcha everything", icon=ICON_TRENDING_UP, icon_bg="
#E6F7EC", icon_color="
#2E9E4F", value_color="
#2E9E4F", card_bg="
#F0FBF3")), unsafe_allow_html=True)
with strength_col2:
st.markdown(clean_html(top_metric_card("Emerging Trends", emerging_count, "Key themes identified", icon=ICON_LIGHTBULB, icon_bg="
#FFF7E0", icon_color="
#F5A623", card_bg="
#FFFBF0")), unsafe_allow_html=True)
st.markdown(f"<div style='height:{SECTION_GAP}px;'></div>", unsafe_allow_html=True)
...
with creator_col1:
st.markdown(clean_html(top_metric_card("Total Creators", creator_count, "Across 2 platforms", icon=ICON_USERS, icon_bg="
#E5F0FF", icon_color="
#2F6BFF", card_bg="
#EFF6FF")), unsafe_allow_html=True)
with creator_col2:
st.markdown(clean_html(top_metric_card("Total Followers", f"{total_followers:,}", "Combined audience", icon=ICON_USER_GROUP, icon_bg="
#EDEBFF", icon_color="
#6F5CFF", card_bg="
#F5F3FF")), unsafe_allow_html=True)
st.markdown(f"<div style='height:{SECTION_GAP}px;'></div>", unsafe_allow_html=True)
...
with demo_kpi1:
st.markdown(clean_html(top_metric_card("Neighbourhoods Profiled", len(demo_work), "AI-researched overviews", icon=ICON_MAP_PIN, icon_bg="
#E5F0FF", icon_color="
#2F6BFF", card_bg="
#EFF6FF")), unsafe_allow_html=True)
with demo_kpi2:
st.markdown(clean_html(top_metric_card("Halal Demand Areas", int(halal_count), "Flagged for halal range", icon=ICON_USERS, icon_bg="
#E6F7EC", icon_color="
#2E9E4F", card_bg="
#F0FBF3")), unsafe_allow_html=True)
with demo_kpi3:
st.markdown(clean_html(top_metric_card("Vegan / Plant-based Areas", int(vegan_count), "Flagged for plant-based range", icon=ICON_GRID, icon_bg="
#EDEBFF", icon_color="
#6F5CFF", card_bg="
#F5F3FF")), unsafe_allow_html=True)
with demo_kpi4:
st.markdown(clean_html(top_metric_card("Premium-Tier Areas", int(premium_count), "Flagged for premium range", icon=ICON_TRENDING_UP, icon_bg="
#FFF3E0", icon_color="
#F4A94E", card_bg="
#FFFBF0")), unsafe_allow_html=True)
st.markdown(f"<div style='height:{TOP_ROW_GAP}px;'></div>", unsafe_allow_html=True)
...
f"""<span style=" display:inline-block; background:
#F5F3FF; color:
#6F5CFF; border:1px solid 
#E2DBFF; font-size:13px; font-weight:700; padding:7px 14px; border-radius:999px; margin:0 8px 8px 0; ">{html.escape(n)}</span>"""
...
<div style="background:
#ffffff; border:1px solid 
#ECEEF3; border-radius:16px; padding:16px 20px; box-shadow:0 8px 20px rgba(17,24,39,0.05);">
<div style="font-size:12px; color:
#9096a3; font-weight:600; margin-bottom:10px;">{len(matching)} MATCHING AREAS</div>
{chips}
</div>
"""),
unsafe_allow_html=True,
...
with roll_kpi1:
st.markdown(clean_html(top_metric_card("Candidates", total_candidates, "In this view", icon=ICON_USERS, icon_bg="
#EDEBFF", icon_color="
#6F5CFF", card_bg="
#F5F3FF")), unsafe_allow_html=True)
with roll_kpi2:
st.markdown(clean_html(top_metric_card("Live", int(live_count), "Fully onboarded", icon=ICON_TRENDING_UP, icon_bg="
#E6F7EC", icon_color="
#2E9E4F", card_bg="
#F0FBF3", value_color="
#2E9E4F")), unsafe_allow_html=True)
with roll_kpi3:
st.markdown(clean_html(top_metric_card("In Progress", int(in_progress_count), "Between contact and live", icon=ICON_MAP, icon_bg="
#FFF3E0", icon_color="
#F4A94E", card_bg="
#FFFBF0")), unsafe_allow_html=True)
with roll_kpi4:
st.markdown(clean_html(top_metric_card("Not Started", int(not_started_count), "Contacted only", icon=ICON_MAP_PIN, icon_bg="
#E5F0FF", icon_color="
#2F6BFF", card_bg="
#EFF6FF")), unsafe_allow_html=True)
st.markdown(f"<div style='height:{SECTION_GAP}px;'></div>", unsafe_allow_html=True)
...
with item_kpi1:
st.markdown(clean_html(top_metric_card("Items Tracked", total_items, "In this view", icon=ICON_GRID, icon_bg="
#EDEBFF", icon_color="
#6F5CFF", card_bg="
#F5F3FF")), unsafe_allow_html=True)
with item_kpi2:
st.markdown(clean_html(top_metric_card("Live Items", int(live_items), "Currently listed", icon=ICON_TRENDING_UP, icon_bg="
#E6F7EC", icon_color="
#2E9E4F", card_bg="
#F0FBF3", value_color="
#2E9E4F")), unsafe_allow_html=True)
with item_kpi3:
st.markdown(clean_html(top_metric_card("Proposed", int(proposed_items), "Awaiting review", icon=ICON_LIGHTBULB, icon_bg="
#FFF7E0", icon_color="
#F5A623", card_bg="
#FFFBF0")), unsafe_allow_html=True)
with item_kpi4:
st.markdown(clean_html(top_metric_card("From Trend Analysis", int(trend_sourced), "Sourced from trend signals", icon=ICON_MAP_PIN, icon_bg="
#E5F0FF", icon_color="
#2F6BFF", card_bg="
#EFF6FF")), unsafe_allow_html=True)
st.markdown(f"<div style='height:{SECTION_GAP}px;'></div>", unsafe_allow_html=True)
...
text = str(text).strip()
match = re.match(r"([\d.]+)\s*(pp)?\s*(higher|lower)\sthan\snon-local(?:\s*
(
[
\d
.
...
color = "
#6F5CFF" if lift_info["direction"] == "higher" else "
#8A8F9C"
bg = "
#F5F3FF" if lift_info["direction"] == "higher" else "
#F3F4F6"
...
<div style="font-size:24px; font-weight:800; color:
#2f3240;">{html.escape(str(local_display))}</div>
<div style="font-size:13px; color:
#9096a3;">vs {html.escape(str(nonlocal_display))} non-local</div>
</div>
"""
return f"""
<div style="
...
">
<div style="font-size:12px; color:
#6b7280; font-weight:600; margin-bottom:6px;">{html.escape(label)}</div>
{values_html}
...
([\d.]+)\s*)?(higher|lower)\svs.?\snon-local",
text, re.IGNORECASE,
)
if not match:
return None
...
<div style="background:
#F5F3FF; border:1px solid 
#E2DBFF; border-radius:14px; padding:14px 20px;">
<div style="font-size:13px; color:
#6F5CFF; font-weight:700;">REPORTING PERIOD</div>
<div style="font-size:15px; color:
#2f3240; margin-top:4px;">
Headline period: <strong>{html.escape(headline_period)}</strong>
· {months_count} complete {month_word} of Local Range performance data tracked so far since launch.
</div>
</div>
"""),
unsafe_allow_html=True,
...
st.markdown(clean_html(top_metric_card("Local Range Orders", fmt_num(lr(context_orders_row)), f"Non-local: {fmt_num(nlr(context_orders_row))}", icon=ICON_MAP_PIN, icon_bg="
#E5F0FF", icon_color="
#2F6BFF", card_bg="
#EFF6FF")), unsafe_allow_html=True)
with context_kpi_cols[1]:
if context_customers_row_early is not None:
st.markdown(clean_html(top_metric_card("Customers Buying Local Range", fmt_num(lr(context_customers_row_early)), f"Non-local: {fmt_num(nlr(context_customers_row_early))}", icon=ICON_USERS, icon_bg="
#EDEBFF", icon_color="
#6F5CFF", card_bg="
#F5F3FF")), unsafe_allow_html=True)
with context_kpi_cols[2]:
if context_attach_row is not None:
st.markdown(clean_html(top_metric_card("% of Site Orders with Local Range", fmt_num(lr(context_attach_row), suffix="%"), "Adoption of Local Range so far", icon=ICON_TRENDING_UP, icon_bg="
#E6F7EC", icon_color="
#2E9E4F", card_bg="
#F0FBF3", value_color="
#2E9E4F")), unsafe_allow_html=True)
...
<div style="background:
#ffffff; border:1px solid 
#ECEEF3; border-radius:14px; padding:18px 22px; box-shadow:0 6px 16px rgba(17,24,39,0.05);">
